...
public class AltaUsuario extends JInternalFrame {
	...
	private JTextField txtpassward;
	...
	public AltaUsuario(IControlador iC) {
		...
		JLabel lblNewLabel_3 = new JLabel("opcional");
		lblNewLabel_3.setBounds(395, 179, 45, 13);
		panelProfe.add(lblNewLabel_3);
		
		JLabel lblNewLabel_3_1 = new JLabel("opcional");
		lblNewLabel_3_1.setBounds(318, 239, 45, 13);
		panelProfe.add(lblNewLabel_3_1);
		...
		btnAceptar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(txtNickname.getText().isEmpty() || txtEmail.getText().isEmpty() || txtApellido.getText().isEmpty() || txtNombre.getText().isEmpty() || dateChooser.getDate()==null){
					...
					if(rbtnSocio.isSelected() || rbtnProfe.isSelected()) {
						if(txtIns.getSelectedIndex()>0 || rbtnSocio.isSelected()) {
							if(!txtDesc.getText().isEmpty() || rbtnSocio.isSelected()) {
								try {
							    	...
							    	if(rbtnProfe.isSelected()) {
							  	    	...
							  	    	String bio = "No cuenta con Bibliografia";
							  	    	String sitweb = "No cuenta con Sitio Web";
							  	    	
							  	    	if(!txtBio.getText().isEmpty()) {
							  	    		bio = txtBio.getText();
							  	    	}
							  	    	if(!txtSitioweb.getText().isEmpty()) {
							  	    		sitweb = txtSitioweb.getText();
							  	    	}

							  	    	dtusuario=new DtProfesor(nick, nombre, apellido, email, fech, null, passward, desc, bio, sitweb, null); 
							    		...
							    		dtusuario=new DtSocio(nick, nombre, apellido, email, fech, null,passward, null);
								...
								JOptionPane.showMessageDialog(null, "Falta la descripcion del Usuario");
							}
						}else {
							JOptionPane.showMessageDialog(null, "Seleccione Intitucion del profesor");
						}
					}else {
		...
		JLabel passward = new JLabel("Contraseña:");
		passward.setFont(new Font("Tahoma", Font.BOLD, 14));
		passward.setBounds(53, 329, 95, 14);
		contentPane.add(passward);
		
		txtpassward = new JTextField();
		txtpassward.setBounds(148, 328, 118, 20);
		contentPane.add(txtpassward);
		txtpassward.setColumns(10);
